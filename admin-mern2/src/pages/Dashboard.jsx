    ...
    <div className="section-spacing">
      ...
      <div className="card-gradient">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-4xl font-bold gradient-text">Dashboard Overview</h1>
            <p className="text-gray-600 mt-2 font-medium text-sm md:text-base">Welcome back! Here's what's happening today.</p>
          </div>
          <div className="md:text-right">
            <p className="text-xs md:text-sm text-gray-600 font-medium">Last Updated</p>
            <p className="text-base md:text-lg font-bold text-gray-900">{new Date().toLocaleDateString()}</p>
          </div>
        </div>
      </div>
            ...
            <div key={index} className="card-gradient hover:shadow-2xl transition-all duration-300 hover:scale-105 group">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-xs md:text-sm font-bold text-gray-600 uppercase tracking-wide">{stat.title}</p>
                  <p className="text-2xl md:text-4xl font-bold text-gray-900 mt-2 md:mt-3 mb-2 md:mb-3">{stat.value}</p>
                  <div className="flex items-center gap-2 mt-2">
                    <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${stat.trendUp ? 'bg-green-100' : 'bg-red-100'}`}>
                      <TrendIcon size={12} className={stat.trendUp ? 'text-green-600' : 'text-red-600'} />
                      <span className={`text-xs font-bold ${stat.trendUp ? 'text-green-600' : 'text-red-600'}`}>
                        {stat.trend}
                      </span>
                    </div>
                    <span className="text-xs text-gray-500 font-medium hidden sm:inline">vs last month</span>
                  </div>
                </div>
                <div className={`${stat.color} p-3 md:p-4 rounded-xl md:rounded-2xl shadow-lg group-hover:scale-110 transition-transform duration-300`}>
      ...
      <div className="card-gradient">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 md:mb-6">
          <h2 className="text-xl md:text-2xl font-bold gradient-text">Recent Orders</h2>
          <button 
            onClick={() => navigate('/orders')}
            className="btn-secondary text-sm hover:scale-105 transition-transform w-full sm:w-auto"
          >
            View All
          </button>
        </div>
        {stats.recentOrders && stats.recentOrders.length > 0 ? (
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          {order.items?.slice(0, 3).map((item, itemIdx) => (
                            <img
                              key={itemIdx}
                              src={item.product?.image || item.product?.images?.[0] || item.image || 'https://via.placeholder.com/40'}
          ...
          <div className="text-center py-12 md:py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border-2 border-gray-200">
            <ShoppingCart className="mx-auto text-gray-400 mb-4" size={40} />
            <p className="text-gray-500 font-bold text-base md:text-lg">No recent orders</p>
            <p className="text-gray-400 text-xs md:text-sm mt-2 px-4">Orders will appear here once customers start purchasing</p>
          </div>
        )}
      </div>
    </div>
